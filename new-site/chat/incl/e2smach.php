...
$query="SELECT * FROM $ext1tble WHERE ID_MEMBER=$ajx_unfo";
$result=neutral_query($query);

if(neutral_num_rows($result)>0){
$user=neutral_fetch_array($result);

print '<div class="s" style="float:right">Simple Machines</div>';

$name=output($user['memberName'],2);
$name=escape($name);
...
$site=output($user['websiteUrl'],2);$site=escape($site);
