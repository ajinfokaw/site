...
$query="SELECT * FROM $ext1tble WHERE uid=$ajx_unfo";
$result=neutral_query($query);

if(neutral_num_rows($result)>0){
$user=neutral_fetch_array($result);

print '<div class="s" style="float:right">exoops</div>';

$name=output($user['uname'],2);
$name=escape($name);
$name=convert_enc($name);

$loc=output($user['user_from'],2);
$loc=escape($loc);
$loc=convert_enc($loc);

print '<b>'.$name.'</b><br /><br />';

$zone=(int)$user['timezone_offset'];
